...
fn worker(addr: String, qos: u8, payload: Arc<Vec<u8>>, stop: Arc<AtomicBool>, count: Arc<AtomicU64>, id: usize) {
    ...
    while !stop.load(Ordering::Relaxed) {
        if stream.write_all(&publish("bench/topic", &payload, qos, pkid)).is_err() {
            break;
        }
        if qos > 0 {
...
fn arg(args: &[String], key: &str, default: &str) -> String {
    args.iter().position(|a| a == key).and_then(|i| args.get(i + 1)).cloned().unwrap_or_else(|| default.to_string())
}

fn main() {
    let args: Vec<String> = std::env::args().collect();
    let addr = args.get(1).cloned().unwrap_or_else(|| "127.0.0.1:1883".to_string());
    ...
    for _ in 0..duration {
        ...
        eprint!("\r  {:.0}s: {:>12} msgs  ({:>10.0} msg/s)", secs, n, n as f64 / secs);
    }
